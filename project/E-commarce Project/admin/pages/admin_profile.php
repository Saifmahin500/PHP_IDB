...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($message)) {
        ...
        // Refresh $admin variable with updated data
        $stmt = $DB_con->prepare("SELECT * FROM admins WHERE id = ?");
        $stmt->execute([$admin_id]);
        $admin = $stmt->fetch(PDO::FETCH_ASSOC);
    }
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <?php if (!empty($admin['photo'])): ?>
                <img src="../uploads/admins/<?= htmlspecialchars($admin['photo']) ?>" width="80" class="rounded mb-2" alt="Profile photo">
            <?php else: ?>
                <img src="../uploads/admins/default.jpg" width="80" class="rounded mb-2" alt="Default photo">
            <?php endif; ?>
